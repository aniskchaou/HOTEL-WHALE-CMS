        ...
        if ( $show_sidebar ) {
            if ( $show_left_sidebar ) {
                ...
                <section id="secondary-left" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                    diaz_show_sidebar( 'page', $globalid, 'left' ); ?>
        ...
        <section id="primary" class="<?php echo esc_attr( $page_layout );?>"><?php
        	...
        	<div class="dt-sc-posts-list-wrapper"><?php
				...
				if( $the_query->have_posts() ):
					...
					$meta = $obj->diaz_post_meta_fields(false, $globalid);
							...
							<article id="post-<?php the_ID();?>" <?php post_class('blog-entry '.$post_style.' '.'format-'.$format);?>><?php
								...
								switch( $post_style ):
									case 'entry-date-left':
										$obj->diaz_post_date_left_style( $post_ID, $meta );
									break;
									case 'entry-date-author-left':
										$obj->diaz_post_date_author_left_style( $post_ID, $meta );
									break;
									...
									default:
										$obj->diaz_post_default_style( $post_ID, $meta );
									break;
								endswitch;
					...
					<h2><?php esc_html_e('Nothing Found.', 'diaz'); ?></h2>
					<p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'diaz'); ?></p><?php
				endif;?>
    		</div>
            <!-- Blog Template -->
        ...
        if ( $show_sidebar ) {
            if ( $show_right_sidebar ) {
                ...
                <section id="secondary-right" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                    diaz_show_sidebar( 'page', $globalid, 'right' ); ?>
