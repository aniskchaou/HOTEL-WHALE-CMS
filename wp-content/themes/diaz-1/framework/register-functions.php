 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('diaz_features')) {

	function diaz_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'diaz-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'diaz-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'diaz-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'diaz-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'diaz-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'diaz-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		}
		...
		register_nav_menus( array(
			'main-menu' => esc_html__('Main Menu', 'diaz'),
		) );
	...
	add_action('after_setup_theme', 'diaz_features');
}

/* ---------------------------------------------------------------------------
 * Check Activate Theme
 * --------------------------------------------------------------------------- */
if (!function_exists('diaz_activation_function')) {
	function diaz_activation_function( $old_name, $old_theme ) {
		...
		$options = array (
		  ...
		  'post-archives-readmore' => '[dt_sc_button title="Read More" style="bordered" /]',
		  ...
		  'breadcrumb_background' => array( 'color' => '#5f5643' ),
		...
		if(!in_array($old_name, array('Diaz', 'Diaz Child'))) {
			update_option( '_cs_options', $options );
		}
	}
	add_action("after_switch_theme", "diaz_activation_function", 10, 2 );
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('diaz_under_construction') ){
	function diaz_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'diaz_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'diaz_rand_css', 101);
	function diaz_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, DIAZ_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('diaz_wp_list_categories') ){
	function diaz_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'diaz_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('diaz_wp_list_archive') ){
	function diaz_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'diaz_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('diaz_wpcf7_form_elements') ){
	function diaz_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'diaz_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Pagination for Blog and Portfolio
 * --------------------------------------------------------------------------- */
function diaz_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'diaz') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'diaz'),
					'next_text'          => esc_html__('Next', 'diaz').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
...
function diaz_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'diaz' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'diaz'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'diaz' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'diaz' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'diaz' );
		}
		...
		$title = esc_html__('Tag Archives','diaz');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function diaz_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('diaz_hex2rgb')) {

	function diaz_hex2rgb($hex) {
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function diaz_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'diaz');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'diaz'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'diaz');
					endif;
					edit_comment_link(esc_html__('Edit', 'diaz'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'diaz'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
function diaz_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function diaz_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function diaz_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $diaz_like_love;
 ...
 * break oembeds.
 * --------------------------------------------------------------------------- */
function diaz_undo_recurrence_oembed_logic() {
...
add_action( 'init', 'diaz_undo_recurrence_oembed_logic' );
...
/* ---------------------------------------------------------------------------
 * Walker Page for diaz_new_wp_page_menu
 * --------------------------------------------------------------------------- */
class DIAZ_Walker_Page extends Walker_Page {
