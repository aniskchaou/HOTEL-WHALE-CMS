...
class diaz_post_functions {

	function __construct() {

		add_action( 'wp_ajax_diaz_post_rating_like', array( $this, 'diaz_post_rating_like' ) );
		add_action( 'wp_ajax_nopriv_diaz_post_rating_like', array( $this, 'diaz_post_rating_like' ) );

	}

	function diaz_post_meta_fields( $ajax = false, $page_ID = 0 ) {
	...
	static function diaz_post_view_count($pid = '') {
	...
	static function diaz_post_like_count($pid = '') {
	...
	function diaz_post_rating_like() {
		...
		if ( wp_verify_nonce( $nonce, 'rating-nonce' ) && $postid > 0 ) {
			...
			$out = esc_html__('Security check', 'diaz');
		}
	...
	function diaz_post_thumb( $post_ID ) {
		...
		elseif( $format === "audio" ) :
			if( has_post_thumbnail($post_ID) ) : ?>
				<a href="<?php echo get_permalink($post_ID);?>" title="<?php printf(esc_attr__('Permalink to %s','diaz'), get_the_title($post_ID));?>">
	...
	function diaz_post_default_style( $post_ID, $meta ) {
        ...
        <div class="entry-thumb">
        	<?php self::diaz_post_thumb( $post_ID ); ?>
        ...
        <div class="entry-details">
            ...
            <div class="entry-meta <?php echo esc_attr($tclass);?>">
            	<div class="date <?php echo esc_attr($meta[6]);?>"><?php esc_html_e('Posted on ','diaz'); echo get_the_date ( get_option('date_format') );?></div>
                ...
                <div class="author <?php echo esc_attr( $meta[5] );?>">
                    ...
                    <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                        title="<?php esc_attr_e('View all posts by ', 'diaz'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                </div>
            </div><!-- Meta -->

            <div class="entry-title">
                <?php if( is_sticky( $post_ID ) ) echo '<span class="sticky-post">'.esc_html__('Featured', 'diaz').'</span>'; ?>
                <h4><a href="<?php echo get_permalink( $post_ID );?>" title="<?php printf(esc_attr__('Permalink to %s','diaz'), the_title_attribute('echo=0'));?>"><?php the_title();?></a></h4>
            </div>

			<?php if( isset($meta[2]) && isset($meta[3]) ):?>
				<div class="entry-body"><?php echo diaz_excerpt($meta[3]);?></div>
            <?php endif;?>
            ...
            <?php if( isset($meta[0]) ):
					...
					if( !empty( $sc ) && shortcode_exists( 'dt_sc_button' ) && diaz_is_plugin_active('js_composer/js_composer.php') ):
						$sc = str_replace("]",' link="url:'.$link.'"]',$sc);
						$sc = do_shortcode($sc);
						echo !empty( $sc ) ? $sc : '';
					else:
						echo '<a href="'.get_permalink().'" title="'.get_the_title().'" class="dt-sc-button small bordered">'.esc_html__('Read More', 'diaz').'</a>';
					endif;
	...
	function diaz_post_date_left_style( $post_ID, $meta ) {
        ...
        <div class="entry-thumb">
            ...
            <div class="entry-title-content">
                ...
                <div class="entry-meta-data <?php echo esc_attr($tclass);?>">
                    <p class="author <?php echo esc_attr( $meta[5] );?>"><?php
                    	esc_html_e('Posted By ', 'diaz'); ?>
                        <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                            title="<?php esc_attr_e('View all posts by ', 'diaz'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                    </p>
                ...
                <!-- Comment -->
                <div class="comments <?php echo esc_attr($meta[7]);?>"><?php
                    comments_popup_link('0'.esc_html__(' Comments', 'diaz'), '1'.esc_html__(' Comment', 'diaz'), '%'.esc_html__(' Comments', 'diaz'), '', esc_html__('Comments off', 'diaz'));?>
                </div><!-- Comment -->
            </div>
		</div><!-- Featured Image -->
	...
	function diaz_post_date_author_left_style( $post_ID, $meta ) {
        ...
        <div class="entry-date-author <?php echo esc_attr($tclass);?>">
            ...
            <div class="entry-author <?php echo esc_attr( $meta[5] );?>">
                ...
                <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                    title="<?php esc_attr_e('View all posts by ', 'diaz'); echo get_the_author();?>"><span><?php echo get_the_author();?></span></a>
            </div>
