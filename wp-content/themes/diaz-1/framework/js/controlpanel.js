...
jQuery(document).ready(function($){
	...
	if($.cookie("diaz_skin")!= null){
		var $href = dttheme_urls.theme_base_url + '/css/skins/' + $.cookie("diaz_skin")+"/style.css";
		$("link[id='skin-css']").attr("href",$href);
		$("ul.color-picker a[id='"+$.cookie("diaz_skin")+"']").addClass("selected");
	}else{
		$("ul.color-picker a:first").addClass("selected");
	}

	//Apply Layout
	if($.cookie("diaz_layout") == "boxed"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("diaz_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");

		$i = ($.cookie("diaz_pattern")) ? $.cookie("diaz_pattern")  : 'pattern1';
		...
		$("ul.pattern-picker a[id="+$.cookie("diaz_pattern")+"]").addClass('selected');
	} else if($.cookie("diaz_layout") == "fullwidth"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("diaz_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");
	...
	if($.cookie("diaz_scheme") != null){
		$("ul.scheme-picker li a").removeAttr("class");
		if( $.cookie("diaz_scheme") === "dark" ) {
		...
		else if( $.cookie("diaz_scheme") === "light" ) {
	...
	$("ul.layout-picker a").click(function(e){
		...
		$.cookie("diaz_layout", $this.attr("id"), { path: '/' });

		if( $.cookie("diaz_layout") === "boxed") {
			$("body").removeClass("layout-wide").addClass("layout-boxed");
			$("div#pattern-holder").slideDown();
			
			if( $.cookie("diaz_pattern") == null ){
				$("ul.pattern-picker a:first").addClass('selected');
				$.cookie("diaz_pattern","pattern1",{ path: '/' });
			}else{
				$("ul.pattern-picker a[id="+$.cookie("diaz_pattern")+"]").addClass('selected');
				$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("diaz_pattern")+".jpg";
	...
	$("ul.scheme-picker a").click(function(e){
		...
		$this.addClass("selected");
		$.cookie("diaz_scheme", $this.attr("id"), { path: '/' });
		if( $.cookie("diaz_scheme") === "dark" ) {
			$("<link id='light-dark-css' href='"+ dttheme_urls.theme_base_url +"/dark/dark-skin.css' rel='stylesheet' media='all' />").insertBefore($('#skin-css'));
		}
		else if( $.cookie("diaz_scheme") === "light" ) {
	...
	$("ul.pattern-picker a").click(function(e){
		if($.cookie("diaz_layout") == "boxed"){
			...
			$("ul.pattern-picker a").removeAttr("class");
			$this.addClass("selected");
			$.cookie("diaz_pattern", $this.attr("id"), { path: '/' });
			$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("diaz_pattern")+".jpg";
			$('body').css('background-image', 'url('+$img+')');
		}
		e.preventDefault();
	...
	$("ul.color-picker a").click(function(e){
		...
		$this.addClass("selected");
		$.cookie("diaz_skin", $this.attr("id"), { path: '/' });
