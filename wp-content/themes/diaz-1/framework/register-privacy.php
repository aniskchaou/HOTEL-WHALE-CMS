...
if( !class_exists( 'diaz_privacy_class' ) ) {

	class diaz_privacy_class {

		function __construct() {

			add_filter( 'comment_form_defaults', array( $this, 'diaz_move_textarea' ) );
			add_action( 'comment_form_top', array( $this, 'diaz_move_textarea' ) );

			// hook privacy message into commentform
			if( cs_get_option('privacy-commentform') == "true" ) {
				add_filter( 'comment_form_default_fields', array( $this, 'diaz_privacy_comment_checkbox' )  );
				add_filter( 'preprocess_comment', array( $this, 'diaz_privacy_verify_comment_checkbox' )  );
			}

			// hook privacy message into mailchimpform
			if( cs_get_option('privacy-subscribeform') == "true" ) {
				add_filter( 'dt_sc_mailchimp_form_elements', array( $this, 'diaz_privacy_mailchimp_checkbox' ) , 10 , 2 );
			}

			// hook privacy message into login/registration forms
			if( cs_get_option('privacy-loginform') == "true" ) {
				add_action( 'login_form', array( $this, 'diaz_privacy_login_extra' ) , 10 , 2 );
				add_action( 'woocommerce_login_form', array( $this, 'diaz_privacy_login_extra' ) , 10 , 2 );
				add_filter( 'wp_authenticate_user', array( $this,'diaz_authenticate_user_acc' ), 99999, 2);
			}

			add_action( 'wp_enqueue_scripts', array( $this, 'diaz_footer_script' ) , 1000 );
			add_action( 'init', array( $this, 'diaz_disable_google_font' ) , 1000 );

			add_action( 'wp_footer', array( $this, 'diaz_print_tracking_code' ), 100 );
		}

		function diaz_move_textarea( $input = array () ) {
			static $textarea = '';
			...
			if ( 'comment_form_defaults' === current_filter() ) {
				...
				$input['label_submit'] = esc_html__('Comment', 'diaz');
				$input['title_reply'] = esc_html__( 'Leave a Comment', 'diaz' );
		 ...
		 * --------------------------------------------------------------------------- */
		function diaz_privacy_comment_checkbox( $comment_field = array() ) {
			...
			$comment_field['comment-form-dt-privatepolicy'] = $this->diaz_privacy_comment_checkbox_content();
		 ...
		 * --------------------------------------------------------------------------- */
		function diaz_privacy_comment_checkbox_content( $content = "", $extra_class = "" ) {
		 ...
		 * --------------------------------------------------------------------------- */
		function diaz_privacy_verify_comment_checkbox( $commentdata ) {

			$post_type = get_post_type($_POST['comment_post_ID']);
			if($post_type!= 'product'){
				if ( ! is_user_logged_in() && ! isset( $_POST['comment-form-dt-privatepolicy'] ) ) {
					$error_message = apply_filters( 'diaz_privacy_comment_checkbox_error_message', __( 'Error: You must agree to our privacy policy to comment on this site...' , 'diaz' ) );
		 ...
		 * --------------------------------------------------------------------------- */
		function diaz_privacy_mailchimp_checkbox( $content = "", $attrs ) {
		 ...
		 * --------------------------------------------------------------------------- */
		function diaz_privacy_login_extra( $form ) {

			$content = do_shortcode( cs_get_option('privacy-loginform-msg') );
			echo ( $this->diaz_privacy_comment_checkbox_content( $content , 'forgetmenot') );
		}

		/* ---------------------------------------------------------------------------
		 *	Authenticate the extra checkbox in the user login screen
		 * --------------------------------------------------------------------------- */
		function diaz_authenticate_user_acc( $user, $password ) {
		    ...
		    if ( isset( $_REQUEST['comment-form-dt-privatepolicy'] ) ) {
		        ...
		        $error = new WP_Error();
		        $error->add('did_not_accept', __( 'You must acknowledge and agree to the privacy policy' , 'diaz'));
		        return $error;
		    }
		}
		 ...
		 * --------------------------------------------------------------------------- */
		function diaz_footer_script() {

			wp_add_inline_script( 'diaz-cookie-js', "function dt_privacy_cookie_setter( cookie_name ) {
		 ...
		 *	Disable Google Font 
		 * --------------------------------------------------------------------------- */
		function diaz_disable_google_font() {
		...
		*	Print Tracking Code
		* --------------------------------------------------------------------------- */
		function diaz_print_tracking_code() {
...
function diaz_privacy_helper() {

	return new diaz_privacy_class();
}
add_action('init', 'diaz_privacy_helper', 20);
 ...
 * -------------------------------------------------------------------------------- */
if( ! function_exists( 'diaz_cookie_consent' ) ) {

    function diaz_cookie_consent() {
        ...
        if( cs_get_option('enable-cookie-consent') == "true" ) {
            ...
            <div class="dt-cookie-consent cookiebar-hidden dt-cookiemessage-<?php echo esc_attr($position); ?>">
	            <div class="container">
					...
					$buttons = diaz_cs_get_option('cookie-bar-buttons', array());
					$i = 0;
					$extra_info = "";

					if( empty( $buttons ) ):
						?><a href="#" class="dt-sc-button filled small dt-cookie-consent-button dt-cookie-close-bar" data-contents="<?php echo esc_attr($cookie_contents); ?>"><?php esc_html_e('OK', 'diaz'); ?></a><?php
					endif;
					...
					foreach($buttons as $button) {
						...
						if( !empty( $button['cookie-button-action'] ) && $button['cookie-button-action'] == 'info_modal' ) {
							...
							$contents[] = array(	'label'		=> __( 'Other external services', 'diaz' ), 
			...
			[dt_sc_privacy_video_embeds]', 'diaz' )
							);

							$wp_privacy_page = get_option('wp_page_for_privacy_policy');
							if( !empty( $wp_privacy_page ) ) {
								$contents[] = array(	'label'		=> __( 'Privacy Policy', 'diaz' ), 
														'content'	=> __( 'You can read about our cookies and privacy settings in detail on our Privacy Policy Page. <br><br> [dt_sc_privacy_link]', 'diaz' ));
							}

							if( cs_get_option('enable-custom-model-content') == "true" ) {
								$contents = diaz_cs_get_option('custom-model-tabs', array());
								$heading  = str_replace("'", "&apos;", diaz_cs_get_option('custom-model-heading', $heading));
							}
    ...
    add_action('wp_footer', 'diaz_cookie_consent', 3);
}