    ...
    class DTHeaderMenuWalker extends Walker_Nav_Menu {
	  ...
	  private $currentParent;
       ...
       public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            $atts['class']   = ! empty( $item->icon_position ) ? 'item-has-icon menu-icon-position-'.$item->icon_position : '';
            ...
            if ($item->object == 'dt_mega_menus' && $depth == 1) {
                ...
                if( !empty( $sc_custom_css ) ) {
                    ...
                    wp_add_inline_style( 'diaz-custom-inline', wp_strip_all_tags( $sc_custom_css ) );
                }

                $custom_css = get_post_meta($item->object_id, '_wpb_post_custom_css', true);
                if( !empty( $custom_css ) ) {
                    wp_enqueue_style( 'diaz-custom-inline' );
                    wp_add_inline_style( 'diaz-custom-inline', wp_strip_all_tags( $custom_css ) );
                }                
            } else {
