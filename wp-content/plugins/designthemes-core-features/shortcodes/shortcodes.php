...
class DTCoreShortcodesDefination {
	...
	function __construct() {
		...
		add_shortcode("dt_sc_animated_heading", array(
			$this,
			"dt_sc_animated_heading"
		) );
		
		add_shortcode("dt_sc_special_heading", array(
			$this,
			"dt_sc_special_heading"
		) );
	...
	function dt_sc_button($attrs, $content = null) {
		...
		if( !empty( $icon_type ) ) {
			...
			if($iconalign == 'btn-icon-left with-icon') {
	...
	function dt_sc_pricing_table_minimal_item($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-pr-tb-col minimal $selected'>";
			...
			$out .= '<div class="dt-sc-tb-header">';
				$out .= '<div class="ptbl-icon-wrapper">';
	...
	function dt_sc_iconbox($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-icon-box {$type} {$class}'{$style}>";
			...
			if( !empty($icon) && ( $icon_type == "icon" || $icon_type == "css_class" ) ):
				$out .= '<div class="dt-sc-icon-wrapper">';
				$out .= "<span class='{$icon}'> </span>";
				$out .= '</div>';
			endif;
			...
			$out .= '<div class="dt-sc-icon-content">';
				if(!empty($subtitle))
					$out .= "<h5>{$subtitle}</h5>";
	...
	function dt_sc_contact_info($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-contact-info {$type} {$class}'>";
			...
			if( $type == 'type5') {
				...
				if( $icon_type == 'icon_url' && !empty($iconurl) ) {
					$image = wpb_getImageBySize( array( 'attach_id' => $iconurl, 'thumb_size' => 'full' ));
					$out .= '<span class="info-image">'.$image['thumbnail'].'</span>';
				}
	...
	function dt_sc_image_caption($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-image-caption {$type} {$class}'>";
			...
			$html_1 = "<div class='dt-sc-image-wrapper'>";
				...
				if( (!empty($icon) || !empty($iconurl) ) && $type != 'type6' ):
					$html_1 .= "<div class='img-cap-icon-wrapper'>";
						if(!empty($icon)):
			...
			$html_2 = "<div class='dt-sc-image-content'>";
				...
				if( (!empty($icon) || !empty($iconurl) ) && $type == 'type6' ):
					$html_2 .= "<div class='img-cap-icon-wrapper'>";
						if(!empty($icon)):
				...
				$html_2 .= "<div class='dt-sc-image-title'>";
					$html_2 .= '<h3>';
						$html_2 .= !empty($a_href) ? "<a href='".$a_href."' title='".$a_title."' target='".$a_target."'>": '';
							$html_2 .= "{$title}";
						$html_2 .= !empty($a_href) ? "</a>" : '';
					$html_2 .= '</h3>';
		...
		if( !empty($images) ) :
			...
			$out = '<div class="dt-sc-images-wrapper">';
				...
				$out .= '<div class="carousel-arrows">';
					...
					$out .= '<p id="pagenumber"><span></span> / '.$i.'</p>';
				...
				$out .= '<article id="post-'.$id.'" class="'.esc_attr($post_classes).'">';
					switch( $style ):
						...
						case 'entry-date-left':
							ob_start();
							$obj->diaz_post_date_left_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'entry-date-author-left':
							ob_start();
							$obj->diaz_post_date_author_left_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						...
						default:
							ob_start();
							$obj->diaz_post_default_style( $id, $meta );
							$out .= ob_get_clean();
						break;
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		$out .= '<div class="downcount '.$class.'" data-date="'.diaz_cs_get_option('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.diaz_cs_get_option('comingsoon-timezone', '0').'">';
	 ...
	 * @return string
	 */
	function dt_sc_animated_heading($attrs, $content = null) {
		extract ( shortcode_atts ( array (
			'heading' => '',
			'bg_image' => '',
			'font_size' => '',
		...
		$out = '<div class="dt-sc-animated-heading '.$class.'">';
			$out .= '<h2 style="'.$style.'">'.$heading.'</h2>';
		$out .= '</div>';
	 ...
	 * @return string
	 */
	function dt_sc_special_heading($attrs, $content = null) {
		extract ( shortcode_atts ( array (
			'title' => '',
			'subtitle' => '',
			'extratitle' => '',
		...
		if( !empty( $google_fonts ) ) {
			...
			if ( ! empty( $params_pairs ) ) {
				foreach ( $params_pairs as $pair ) {
					$param = preg_split( '/\:/', $pair );
					if ( ! empty( $param[0] ) && isset( $param[1] ) ) {
						$result[ isset($param[0]) ] = rawurldecode( isset($param[1]) );
					}
				}
			}
		...
		$out = '<div class="dt-sc-special-heading '.$class.' '.$align.'">';
			$out .= '<h2>'.$title.'</h2>';
			$out .= '<p>'.$subtitle.'</p>';
			$out .= '<h5 '.$inline_style.'>'.$extratitle.'</h5>';
		$out .= '</div>';
